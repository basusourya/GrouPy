...
#===================================================================== Dataloaders =================================================================================================
def kmnist_transform_array(aug_dict=aug_dict, aug_array=torch.zeros(6), seed=12345):
...
def kmnist_trainloader(aug_dict=aug_dict, aug_array=aug_array, seed=12345, train_batch_size = 64, mini_trainsize = 4000, mini_train=True):
  torch.manual_seed(seed)
  transform = transforms.Compose(kmnist_transform_array(aug_dict=aug_dict, aug_array=aug_array, seed=seed))
...
def kmnist_testloader(aug_dict=aug_dict, aug_array=aug_array, seed=12345, test_batch_size = 1000, mini_testsize = 1000, mini_test=True):
  torch.manual_seed(seed)
  testtransform = transforms.Compose(kmnist_transform_array(aug_dict=aug_dict, seed=seed))
  testset = torchvision.datasets.KMNIST(root='./data', train=False, download=True, transform=testtransform)
...
class Environment(object):
  ...
  def get_state_accuracy(self, state):
    ...
    trainloader = kmnist_trainloader(aug_dict=aug_dict, aug_array=train_aug_array, seed=12345, train_batch_size = self.child_batch_size, mini_trainsize = self.child_train_size, mini_train=True)
    testloader = kmnist_testloader(aug_dict=aug_dict, aug_array=test_aug_array, seed=12345, test_batch_size = self.child_test_batch_size, mini_testsize = self.child_test_size, mini_test=True)
    testaccuracy = 0
...
def plot_steps_update(x_steps, y_accuracy_per_step):
  ...
  plt.pause(0.001)  #pause a bit so that plots are updated
  fig.savefig('GNAS_GCNN_steps_KMNIST.eps', format='eps', dpi=1000)
...
def main():
  ...
  parser = argparse.ArgumentParser(description='Deep Q-learning KMNIST')
  ...
  while env.models_trained < args.max_models and num_episodes < args.max_episodes:
    ...
    if env.models_trained % Q_TARGET_UPDATE == 0:
      target_net.load_state_dict(policy_net.state_dict())
      torch.save(target_net.state_dict(), "Target_Net_KMNIST_DQN")

  #plot_overall(x_models_trained, y_accuracy, args.aug_array_id)

  # Save network and plot data
  np.save("y_accuracy_KMNIST",y_accuracy)
  np.save("x_models_KMNIST",x_models_trained)
  np.save("steps_per_model_KMNIST",steps_per_model_list)
  np.save("model_accuracies_KMNIST",env.visited_model_accuracies)
  np.save("model_rewards_KMNIST",env.visited_model_rewards)
  torch.save(policy_net.state_dict(), "KMNIST_DQN")
