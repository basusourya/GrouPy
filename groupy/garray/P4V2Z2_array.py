...
# A transformation in p4v2z2 can be coded using four integers:
...
# while the integer parameterization is required when indexing gfunc on p4v2z2.


class P4V2Z2Array(MatrixGArray):
    ...
    def __init__(self, data, p='int'):
        ...
        self._left_actions[P4V2Z2Array] = self.__class__.left_action_hmat
        self._left_actions[Z2Array] = self.__class__.left_action_hvec

        super(P4V2Z2Array, self).__init__(data, p)
    ...
    def int2hmat(self, int_data):
        ...
        out[..., 0, 2] = u
        out[..., 1, 0] = np.sin(0.5 * np.pi * r) * (-1) ** m
        out[..., 1, 1] = np.cos(0.5 * np.pi * r) * (-1) ** m
        out[..., 1, 2] = v
        out[..., 2, 2] = 1.
        return out
...
def identity(shape=(), p='int'):
    e = P4V2Z2Array(np.zeros(shape + (4,), dtype=np.int), 'int')
...
def rand(minu, maxu, minv, maxv, size=()):
    ...
    return P4V2Z2Array(data=data, p='int')
...
def meshgrid(m=m_range(), r=r_range(), u=u_range(), v=v_range()):
    m = P4V2Z2Array(m.data[:, None, None, None, ...], p=m.p)
    r = P4V2Z2Array(r.data[None, :, None, None, ...], p=r.p)
    u = P4V2Z2Array(u.data[None, None, :, None, ...], p=u.p)
    v = P4V2Z2Array(v.data[None, None, None, :, ...], p=v.p)
    return u * v * m * r
