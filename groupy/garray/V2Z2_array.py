...
# A transformation in v2z2 can be coded using four integers:
...
class V2Z2Array(MatrixGArray):
    ...
    def __init__(self, data, p='int'):
        ...
        super(V2Z2Array, self).__init__(data, p)
...
def identity(shape=(), p='int'):
    e = V2Z2Array(np.zeros(shape + (3,), dtype=np.int), 'int')
...
def rand(minu, maxu, minv, maxv, size=()):
    ...
    return V2Z2Array(data=data, p='int')
...
def meshgrid(m=m_range(), r=r_range(), u=u_range(), v=v_range()):
    m = V2Z2Array(m.data[:, None, None, None, ...], p=m.p)
    r = V2Z2Array(r.data[None, :, None, None, ...], p=r.p)
    u = V2Z2Array(u.data[None, None, :, None, ...], p=u.p)
    v = V2Z2Array(v.data[None, None, None, :, ...], p=v.p)
    return u * v * m * r
