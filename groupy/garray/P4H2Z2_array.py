...
# A transformation in p4h2z2 can be coded using four integers:
...
class P4H2Z2Array(MatrixGArray):
    ...
    def __init__(self, data, p='int'):
        ...
        super(P4H2Z2Array, self).__init__(data, p)
...
def identity(shape=(), p='int'):
    e = P4H2Z2Array(np.zeros(shape + (4,), dtype=np.int), 'int')
...
def rand(minu, maxu, minv, maxv, size=()):
    ...
    return P4H2Z2Array(data=data, p='int')
...
def meshgrid(m=m_range(), r=r_range(), u=u_range(), v=v_range()):
    m = P4H2Z2Array(m.data[:, None, None, None, ...], p=m.p)
    r = P4H2Z2Array(r.data[None, :, None, None, ...], p=r.p)
    u = P4H2Z2Array(u.data[None, None, :, None, ...], p=u.p)
    v = P4H2Z2Array(v.data[None, None, None, :, ...], p=v.p)
    return u * v * m * r
